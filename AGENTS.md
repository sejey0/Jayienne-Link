...
- **Cancel / Dismiss Secondary Action Buttons (e.g. Cancel, Dismiss):**
  - **NEVER use plain unbordered, transparent, or uncolored text buttons** for Cancel actions.
  - Must consistently apply the signature rich rose-crimson alert gradient and styling:
    - Gradient: `LinearGradient(colors: [Color(0xFFFF5252), Color(0xFFD81B60)], begin: Alignment.topLeft, end: Alignment.bottomRight)`
    - Shadow: `BoxShadow(color: Color(0xFFFF5252).withValues(alpha: 0.35), blurRadius: 8, offset: Offset(0, 2))`
    - Foreground: Crisp white text with `FontWeight.bold` and `Icon(Icons.close_rounded, color: Colors.white)`
...
- **Modal Design Standards:**
  ...
  - **Cancel / Dismiss Action:** `SecondaryCancelButton` with signature rose-crimson alert gradient, glowing shadow, and close icon.
